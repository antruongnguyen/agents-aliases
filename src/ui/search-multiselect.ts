// Interactive fuzzy-search multiselect prompt.
// Adapted from vercel-labs/skills (src/prompts/search-multiselect.ts), MIT License,
// Copyright (c) 2025 Vercel — https://github.com/vercel-labs/skills
...
export interface SearchMultiselectOptions<T> {
  ...
  selectAll?: boolean;
  /** Plural noun used in hint copy, e.g. "agents". Defaults to "items". */
  itemNoun?: string;
}
          ...
          if (includeDetail) {
            ...
            const detail =
              hasSelectAll && cursor === 0
                ? `Select or clear all ${items.length} ${itemNoun}.`
                : entry?.type === "group"
                  ? `Select all ${entry.items.length} ${itemNoun} in ${entry.group}.`
                  : entry?.item.detail;
