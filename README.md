...
- **Rules** → generated adapters: your canonical rule file's body plus each tool's required frontmatter (`globs`/`alwaysApply` for Cursor MDC, `applyTo` for Copilot, `trigger` for Windsurf), stamped with a `<!-- GENERATED by agents-aliases -->` marker.
